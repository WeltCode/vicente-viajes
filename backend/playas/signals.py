...
def delete_playa_image_on_delete(sender, instance, **kwargs):
    """Elimina la imagen de Cloudflare cuando se borra una playa."""
    if instance.image:
        _destroy_cf_image(str(instance.image))


@receiver(pre_save, sender=Playa)
def delete_old_playa_image_on_update(sender, instance, **kwargs):
    """Elimina la imagen anterior de Cloudflare cuando se reemplaza por una nueva."""
    if not instance.pk:
        return
    try:
        old = Playa.objects.get(pk=instance.pk)  # pyright: ignore[reportAttributeAccessIssue]
